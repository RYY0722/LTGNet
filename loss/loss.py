...
class TPerceptualLoss(nn.Module):
    ...
    def forward(self, map_lv3, map_lv2, map_lv1, S, T_lv3, T_lv2, T_lv1):
        ...
        if (self.type == 'l1'):
            ...
            S_lv3 = torch.sqrt(S_lv3)
            S_lv2 = torch.sqrt(S_lv2)
            S_lv1 = torch.sqrt(S_lv1)
...
class AdversarialLoss(nn.Module):
    def __init__(self, logger, use_cpu=False, num_gpu=1, gan_type='WGAN_GP', gan_k=1, 
        ...
        self.discriminator = discriminator.Discriminator(train_crop_size).to(self.device)
        if (num_gpu > 1):
...
def get_loss_dict(args, logger):
    ...
    if (abs(args.ref_w - 0) > 1e-8):
        loss['ref_loss'] = TPerceptualLoss()
