...
class MSFP2(nn.Module):
    def __init__(self, n_feats):
        super(MSFP2, self).__init__()
...
class MSFP3(nn.Module):
    def __init__(self, n_feats):
        super(MSFP3, self).__init__()
        self.conv12 = conv1x1(n_feats, n_feats)
...
class Decoder(nn.Module):
    def __init__(self, num_res_blocks, n_feats, res_scale):
        ...
        self.convlv2 = conv3x3(64, n_feats)
        self.convlv3 = conv3x3(128, n_feats)
        ### stage11
        ...
        ### stage21, 22
        self.conv22_head = conv3x3(128+n_feats*2, n_feats)

        self.ex12 = MSFP2(n_feats)
        ...
        ### stage31, 32, 33
        self.conv33_head = conv3x3(64+n_feats*2, n_feats)

        self.ex123 = MSFP3(n_feats)
    ...
    def forward(self, x, lrsr_lv2, lrsr_lv3, T_lv3=None, T_lv2=None, T_lv1=None):
        ### shallow feature extraction
        lrsr_lv1 = self.SFE(x)
        lrsr_lv2 = self.convlv2(lrsr_lv2)
        lrsr_lv3 = self.convlv3(lrsr_lv3)
        ...
        x21_res = x21
        x22 = self.conv12(x11)
        x22_up = F.relu(self.ps12(x22))
        
        ### soft-attention
        x22 = lrsr_lv2
        x22_res = lrsr_lv2
        x22_res = torch.cat((x22_res, T_lv2, x22_up), dim=1)
        ...
        x32_res = x32
        x33 = self.conv23(x22)
        x33_up = F.relu(self.ps23(x33))
        x33 = lrsr_lv1
        ### soft-attention
        x33_res = x33
        x33_res = torch.cat((x33_res, T_lv1, x33_up), dim=1)
